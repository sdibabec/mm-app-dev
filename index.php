  ...
  if (x.style.display === "none") {
            ...
            $.ajax({
              ...
              success: function(data){
                  ...
                  if(data.exito==1)
                  {
                      ...
                      //    }, 3000);
                      //setTimeout(function(){ 
                          window.location="<?=(($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $clSistema->seccionPadre($_GET['tCodSeccion']) );?>"; 
    ...
    //otros
        ...
        function soloNumeros(objeto)
				{
					var cadena=document.getElementById(objeto).value,
						arreglo,
						bandera=0,
						texto=cadena;
          			...
          			if(cadena.includes("."))
            		{
            			...
            			if(cuenta.length>2)
            			{
            			  ...
            			  for(var i=0;i<(cadena.length-1);i++)
    						{
            			    	document.getElementById(objeto).value +=cadena[i];
            			    }       
            			}
            		}
          				if(bandera==0)
          				...
          				for(var i=0;i<cadena.length;i++)
    						{
    						var reemplazar=cadena[i];
    						var comparar=parseInt(cadena[i]);
    							if((Number.isInteger(comparar)==false))
    							{
                    	    		if(cadena[i]!=".")
                    	    		{
                    	    			document.getElementById(objeto).value="";
                    	    			for(var j=0;j<i;j++)
                    	    				{
                    	    					document.getElementById(objeto).value +=cadena[j];
                    	    				}
                    	    		}
    							}
    						}
