...
$eCodUsuario = $data->eCodUsuario ? $data->eCodUsuario : false;
        ...
        $tRFC = $data->tRFC ? "'".utf8_encode($data->tRFC)."'" : "NULL";
        $tRazonSocial = $data->tRazonSocial ? "'".utf8_encode($data->tRazonSocial)."'" : "NULL";
        $tDomicilioFiscal = $data->tDomicilioFiscal ? "'".utf8_encode($data->tDomicilioFiscal)."'" : "NULL";
...
if(!sizeof($errores))
{
        if(!$eCodUsuario)
        {
            $insert = "INSERT INTO SisUsuarios (tTitulo, tTelefonoFijo, tTelefonoMovil, tEstado, tNombre, tApellidos, tCorreo, tPasswordAcceso, tPasswordOperaciones,  eCodEstatus, eCodPerfil, fhFechaCreacion,bAll, eCodEntidad,tRFC,tRazonSocial,tDomicilioFiscal) VALUES ($tTitulo, $tTelefonoFijo, $tTelefonoMovil, $tEstado, $tNombre, $tApellidos, $tCorreo, $tPasswordAcceso, $tPasswordOperaciones, 3, $eCodPerfil, $fhFechaCreacion,$bAll, $eCodEntidad,$tRFC,$tRazonSocial,$tDomicilioFiscal)";
        }
        else
        {
            ...
            tDomicilioFiscal=$tDomicilioFiscal,
            eCodEstatus=3
            WHERE
